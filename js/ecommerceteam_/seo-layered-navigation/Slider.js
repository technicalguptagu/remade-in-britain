...
function S(unique, input, cfg) {
    ...
    function _adjustThumb () {
		...
		symbol = document.getElementById('currentCurrencySymbol').value;
		var _currentReal = _current + _min;
		//alert(_currentReal+"=="+_isOrdinal+"=="+_valueText);		
        _thumb.style[_off.toLowerCase()] = S.makeLength(newPos, _rl);
        if (_dynLabel) {
            _dynLabel.firstChild.firstChild.nodeValue = _isOrdinal ? _sMarks.labels[_current] :  symbol+_currentReal + " " + _valueText;
