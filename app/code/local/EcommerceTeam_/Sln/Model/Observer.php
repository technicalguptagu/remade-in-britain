...
class EcommerceTeam_Sln_Model_Observer
{
                        ...
                        if ($model = $attributes->getItemByColumnValue('attribute_code', $code)) {
                            ...
                            if ($model->getFrontendInput() == 'select' || $model->getFrontendInput() == 'multiselect') {
                                ...
                                foreach ((array)$value as $_value) {
                                    ...
                                    if (isset($_value[0]) && isset($_value[1])) {
                                        $_labelValues[] = Mage::helper('core')->currency(($_value[0]-1)*$_value[1], true, false) .' - '. Mage::helper('core')->currency(($_value[0])*$_value[1], true, false);
                                    }
                                }
                            }
                            ...
                            if ($categories && !empty($categories)) {
                                ...
                                foreach ((array)$value as $id) {
                                    $_labelValues[] = $categories->getItemById($id)->getName();
                                }
    ...
    public function loadAttributeData($event)
    {
        ...
        $select->from($table, array('group_id', 'frontend_type','top_category', 'comment'));
    ...
    public function saveAttributeData($event)
    {
        ...
        $topCategory    = addslashes($event->getAttribute()->getData('top_category'));
