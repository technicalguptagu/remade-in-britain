...
abstract class EcommerceTeam_Sln_Block_Layer_Filter_Abstract extends Mage_Catalog_Block_Layer_Filter_Abstract {
    ...
    public function getRemoveUrl() {
        ...
        if (!empty($values) && is_array($values)) {
            ...
            if (!empty($values)) {
                foreach ($values as $_requestVar => $_requestValue) {
                    ...
                    $filterParams[] = implode(',', $_requestValue);
                }
                ksort($filterParams);
            }
        }
        ...
        if (Mage::app()->getFrontController()->getRequest()->getModuleName() == 'catalogsearch') {
            ...
            return sprintf('%s/filter/%s', trim($baseUrl, '/'), implode('/', $filterParams));
        }
    }
    ...
    public function getTopCategory($attributeName) {
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($attributeName == $attribute->getFrontendLabel()) {
                return $attribute->getTopCategory();
            }
        }
     ...
     public function isThisTopCategory() {
     
        $cat = Mage::registry('current_category');
        if (is_object($cat) && (int) $cat->getId() > 0) {
            if ($cat->getLevel() == 2) {
                return true;
            }            
        }
